...
export function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Detect if we're over the hero section with image/video
      const heroSection = document.querySelector('[data-hero-section="true"]');
      if (heroSection) {
        const heroRect = heroSection.getBoundingClientRect();
        const headerHeight = headerRef.current?.offsetHeight || 0;

        // Check if the header is still within the hero section
        setIsOverImage(heroRect.bottom > headerHeight);
      }
    };
    ...
    <header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || isMobileMenuOpen
          ...
          : isOverImage
          ? "bg-black/25 dark:bg-black/30 backdrop-blur-sm"
